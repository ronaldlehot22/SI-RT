...
@section('header_title', 'Tambah Kartu Keluarga')
@section('back')
@endsection
@section('back_url', route('keluarga.index'))

@section('content')
<form action="{{ route('keluarga.store') }}" method="POST" class="px-4 pt-4 pb-8 space-y-5">
    @csrf
    ...
    <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="no_kk" value="{{ old('no_kk') }}" maxlength="16" inputmode="numeric"
    ...
    <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-4">
        ...
        <p class="text-xs text-gray-400 -mt-2">RT/RW dan wilayah sudah terisi otomatis untuk RT 012 RW 005.</p>

        <div>
            <label class="block text-xs font-semibold text-gray-500 mb-1.5">Alamat (No. Rumah / Detail) <span class="text-red-400">*</span></label>
            <input type="text" name="alamat" value="{{ old('alamat') }}"
                   placeholder="Contoh: Jl. Nekamase No. 5"
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                ...
                <input type="number" name="rt" value="{{ old('rt', 12) }}" min="1"
            ...
            <div>
                ...
                <input type="number" name="rw" value="{{ old('rw', 5) }}" min="1"
                       class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm bg-hijau-50 text-hijau-700 font-bold">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="kelurahan" value="{{ old('kelurahan', 'Liliba') }}"
        ...
        <div>
            ...
            <input type="text" name="kecamatan" value="{{ old('kecamatan', 'Oebobo') }}"
                   class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm bg-gray-50">
        </div>

        <div>
            <label class="block text-xs font-semibold text-gray-500 mb-1.5">Kab/Kota</label>
            <input type="text" name="kab_kota" value="{{ old('kab_kota', 'Kota Kupang') }}"
                   class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm bg-gray-50">
        </div>

        <div>
            <label class="block text-xs font-semibold text-gray-500 mb-1.5">Provinsi</label>
            <input type="text" name="provinsi" value="{{ old('provinsi', 'Nusa Tenggara Timur') }}"
                   class="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm bg-gray-50">
        </div>

        <div>
            <label class="block text-xs font-semibold text-gray-500 mb-1.5">Kode Pos</label>
            <input type="text" name="kode_pos" value="{{ old('kode_pos') }}" maxlength="5" inputmode="numeric"
                   placeholder="5 digit"
